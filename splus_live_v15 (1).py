...
def install_csp_bypass(context):
    """
    توقیف تمام پاسخ‌های HTTP از splus.ir و تعدیل هدر CSP
    تا blob: Workers و inline scripts اجازه کار بگیرند.
    ...
    # Intercept all requests to splus.ir
    context.route("**/*", _csp_handler)
    log("  CSP bypass نصب شد (route interception)")
    return True
